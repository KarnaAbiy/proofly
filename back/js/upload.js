...
async function sendToTelegram(text, files = []) {
  try {
    // 1. Отправка текста всем получателям
    ...
    // 2. Отправка файлов по MIME-типу
    if (files.length > 0) {
      for (const file of files) {
        ...
        for (const chat_id of CHAT_IDS) {
          ...
          if (mime === "image/jpeg" || mime === "image/png") {
            ...
            // fallback на document (если вдруг попадёт другой тип)
...
// === 💬 Проверка на мат и бессмысленный текст ===
const badWords = [
