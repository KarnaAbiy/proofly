...
app.post('/complaints', upload.array('mediaUpload', 6), async (req, res) => {
  ...
  for (const chatId of subscribers) {
    try {
      const mediaItems = fileUrls
        .filter(url => {
          const ext = path.extname(url).toLowerCase();
          return ['.jpg', '.jpeg', '.png', '.gif', '.mp4', '.mov', '.avi', '.mkv'].includes(ext);
        })
        .map(url => {
          const ext = path.extname(url).toLowerCase();
          const type = ['.mp4', '.mov', '.avi', '.mkv'].includes(ext) ? 'video' : 'photo';
          return {
            type,
            media: url,
            caption: text,
